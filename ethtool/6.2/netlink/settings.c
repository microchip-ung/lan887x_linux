...
static const char *const names_link_tc10_state[] = {
	[ETHTOOL_TC10_PHY_NORMAL]               = "Normal",
	[ETHTOOL_TC10_PHY_SLEEP_ACK]		= "Sleep Acknowledgment",
	[ETHTOOL_TC10_PHY_SLEEP_REQ]            = "Sleep Request",
	[ETHTOOL_TC10_PHY_SLEEP_FAIL]           = "Sleep Fail",
	[ETHTOOL_TC10_PHY_SLEEP_SILENT]         = "Sleep Silent",
	[ETHTOOL_TC10_PHY_SLEEP]                = "Sleep",
	[ETHTOOL_TC10_PHY_UNKNOWN]              = "Unknown",
};
...
int tc10_reply_cb(const struct nlmsghdr *nlhdr, void *data)
{
	const struct nlattr *tb[ETHTOOL_A_TC10_MAX + 1] = {};
	struct nl_context *nlctx = data;
	DECLARE_ATTR_TB_INFO(tb);
	int ret;
	...
	if (ret < 0)
		return ret;
	nlctx->devname = get_dev_name(tb[ETHTOOL_A_TC10_HEADER]);
	if (!dev_ok(nlctx))
		return MNL_CB_OK;

	if (tb[ETHTOOL_A_TC10_STATE]) {
		const char *link_tc10_state_str;
		uint8_t val = mnl_attr_get_u8(tb[ETHTOOL_A_TC10_STATE]);

		print_banner(nlctx);
		link_tc10_state_str = get_enum_string(names_link_tc10_state,
						      ARRAY_SIZE(names_link_tc10_state),
						      val);
		printf("\tTC10 State: %s\n", link_tc10_state_str);
...
int nl_gset(struct cmd_context *ctx)
{
	...
	if (netlink_cmd_check(ctx, ETHTOOL_MSG_LINKMODES_GET, true) ||
	    ...
	    netlink_cmd_check(ctx, ETHTOOL_MSG_TC10_STATE_GET, true))
		return -EOPNOTSUPP;
	...
	ret = gset_request(nlctx, ETHTOOL_MSG_TC10_STATE_GET, ETHTOOL_A_TC10_HEADER,
			   tc10_reply_cb);
	if (ret == -ENODEV)
		return ret;
